...
async function fetchOnceAndSaveToDiskWithBuffer(dir, filename) {
...
async function loadImages(dir, filename) {
  const buffer = await fetchOnceAndSaveToDiskWithBuffer(dir, filename);
...
async function loadLabels(dir, filename) {
  const buffer = await fetchOnceAndSaveToDiskWithBuffer(dir, filename);
...
class MnistDataset {
  ...
  async loadData(directory) {
    this.dataset = await Promise.all([
      loadImages(directory, TRAIN_IMAGES_FILE), loadLabels(directory, TRAIN_LABELS_FILE),
      loadImages(directory, TEST_IMAGES_FILE), loadLabels(directory, TEST_LABELS_FILE)
    ]);
